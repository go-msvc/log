package log

import (
	"bytes"
	"errors"
	"fmt"
)

//Level determines logger priority. Higher levels are more important.
type Level int8

const (
	//TraceLevel prints details values, often hex dumps
	TraceLevel Level = iota - 2
	//DebugLevel shows the sequence flow through the program code
	DebugLevel Level = iota - 1
	//InfoLevel is the default logging priority (value 0) to show progress and status
	InfoLevel
	//WarnLevel alerts the operator of potential issues
	WarnLevel
	//ErrorLevel describes errors, things that needs attention or breaks operation
	ErrorLevel
	//??DPanicLevel logs are particularly important errors. In development the
	//logger panics after writing the message.

	//PanicLevel logs then panic (temrminate the program)
	PanicLevel
	//FatalLevel logs a message, then calls os.Exit(1).
	FatalLevel

	// //internal levels:
	// parentLevel

	_minLevel = TraceLevel
	_maxLevel = FatalLevel
)

var errUnmarshalNilLevel = errors.New("can't unmarshal a nil *logger.Level")

//String value of level in lowercase
func (l Level) String() string {
	switch l {
	case TraceLevel:
		return "trace"
	case DebugLevel:
		return "debug"
	case InfoLevel:
		return "info"
	case WarnLevel:
		return "warn"
	case ErrorLevel:
		return "error"
	// case DPanicLevel:
	// 	return "dpanic"
	case PanicLevel:
		return "panic"
	case FatalLevel:
		return "fatal"
	default:
		return fmt.Sprintf("Level(%d)", l)
	}
} //Level.String()

//STRING is uppercase of String() but faster than ToUpper(Level.String())
func (l Level) STRING() string {
	switch l {
	case TraceLevel:
		return "TRACE"
	case DebugLevel:
		return "DEBUG"
	case InfoLevel:
		return "INFO"
	case WarnLevel:
		return "WARN"
	case ErrorLevel:
		return "ERROR"
	// case DPanicLevel:
	// 	return "DPANIC"
	case PanicLevel:
		return "PANIC"
	case FatalLevel:
		return "FATAL"
	default:
		return fmt.Sprintf("LEVEL(%d)", l)
	}
} //Level.CapitalString()

//MarshalText marshals the Level to text. Note that the text representation
//drops the -Level suffix (see example).
func (l Level) MarshalText() ([]byte, error) {
	return []byte(l.String()), nil
}

//UnmarshalText unmarshals text to a level. Like MarshalText, UnmarshalText
// expects the text representation of a Level to drop the -Level suffix (see
// example).
//
// In particular, this makes it easy to configure logging levels using YAML,
// TOML, or JSON files.
func (l *Level) UnmarshalText(text []byte) error {
	if l == nil {
		return errUnmarshalNilLevel
	}
	if !l.unmarshalText(text) && !l.unmarshalText(bytes.ToLower(text)) {
		return fmt.Errorf("unrecognized level: %q", text)
	}
	return nil
} //Level.UnmarshalText()

func (l *Level) unmarshalText(text []byte) bool {
	switch string(text) {
	case "trace", "TRACE":
		*l = TraceLevel
	case "debug", "DEBUG":
		*l = DebugLevel
	case "info", "INFO", "": // make the zero value useful
		*l = InfoLevel
	case "warn", "WARN":
		*l = WarnLevel
	case "error", "ERROR":
		*l = ErrorLevel
	// case "dpanic", "DPANIC":
	// 	*l = DPanicLevel
	case "panic", "PANIC":
		*l = PanicLevel
	case "fatal", "FATAL":
		*l = FatalLevel
	default:
		return false
	}
	return true
} //unmarshalText()

// Set sets the level for the flag.Value interface.
func (l *Level) Set(s string) error {
	return l.UnmarshalText([]byte(s))
}

// Get gets the level for the flag.Getter interface.
func (l *Level) Get() interface{} {
	return *l
}

// Enabled returns true if the given level is at or above this level.
func (l Level) Enabled(lvl Level) bool {
	return lvl >= l
}

// LevelEnabler decides whether a given logging level is enabled when logging a
// message.
//
// Enablers are intended to be used to implement deterministic filters;
// concerns like sampling are better implemented as a Core.
//
// Each concrete Level value implements a static LevelEnabler which returns
// true for itself and all higher logging levels. For example WarnLevel.Enabled()
// will return true for WarnLevel, ErrorLevel, DPanicLevel, PanicLevel, and
// FatalLevel, but return false for InfoLevel and DebugLevel.
type LevelEnabler interface {
	Enabled(Level) bool
}
